CREATE TABLE agent_learning_report (
...
CREATE INDEX idx_agent_learning_report_workspace_issue
    ON agent_learning_report(workspace_id, issue_id, created_at DESC);

CREATE INDEX idx_agent_learning_report_workspace_agent
    ON agent_learning_report(workspace_id, agent_id, created_at DESC);

CREATE TABLE agent_evolution_suggestion (
    id uuid PRIMARY KEY DEFAULT gen_random_uuid(),
...
CREATE INDEX idx_agent_evolution_suggestion_report
    ON agent_evolution_suggestion(report_id, created_at ASC);

CREATE INDEX idx_agent_evolution_suggestion_workspace_status
    ON agent_evolution_suggestion(workspace_id, status, created_at DESC);

CREATE TABLE agent_evolution_application (
    id uuid PRIMARY KEY DEFAULT gen_random_uuid(),
...
CREATE INDEX idx_agent_evolution_application_suggestion
    ON agent_evolution_application(suggestion_id, created_at DESC);

CREATE INDEX idx_agent_evolution_application_workspace
