...
BEGIN
    IF NOT EXISTS (
        SELECT 1 FROM pg_constraint WHERE conname = 'skill_workspace_id_id_unique'
    ) THEN
        ALTER TABLE skill
...
BEGIN
    IF NOT EXISTS (
        SELECT 1 FROM pg_constraint WHERE conname = 'issue_integration_default_skill_workspace_fk'
    ) THEN
...
CREATE INDEX IF NOT EXISTS issue_integration_workspace_idx
    ON issue_integration (workspace_id, provider);

CREATE TABLE IF NOT EXISTS issue_sync_config (
    id UUID PRIMARY KEY DEFAULT gen_random_uuid(),
    workspace_id UUID NOT NULL REFERENCES workspace(id) ON DELETE CASCADE,
...
CREATE INDEX IF NOT EXISTS issue_sync_config_integration_idx
    ON issue_sync_config (integration_id);

CREATE INDEX IF NOT EXISTS issue_sync_config_poll_idx
