CREATE TABLE issue_draft_session (
...
CREATE TABLE issue_draft_message (
    id uuid PRIMARY KEY DEFAULT gen_random_uuid(),
...
CREATE TABLE issue_draft_member_task (
    id uuid PRIMARY KEY DEFAULT gen_random_uuid(),
    session_id uuid NOT NULL REFERENCES issue_draft_session(id) ON DELETE CASCADE,
    workspace_id uuid NOT NULL REFERENCES workspace(id) ON DELETE CASCADE,
...
CREATE TABLE issue_draft_artifact (
    id uuid PRIMARY KEY DEFAULT gen_random_uuid(),
    session_id uuid NOT NULL REFERENCES issue_draft_session(id) ON DELETE CASCADE,
    workspace_id uuid NOT NULL REFERENCES workspace(id) ON DELETE CASCADE,
...
CREATE TABLE issue_draft_confirm_step (
    session_id uuid NOT NULL REFERENCES issue_draft_session(id) ON DELETE CASCADE,
    workspace_id uuid NOT NULL REFERENCES workspace(id) ON DELETE CASCADE,
    step text NOT NULL,
...
CREATE INDEX idx_issue_draft_session_workspace_created
    ON issue_draft_session (workspace_id, created_at DESC);
CREATE INDEX idx_issue_draft_message_session_created
    ON issue_draft_message (session_id, created_at ASC);
CREATE INDEX idx_issue_draft_member_task_session
    ON issue_draft_member_task (session_id, created_at ASC);
CREATE INDEX idx_issue_draft_artifact_session_type_revision
