...
export function rowMatchesFilters(
  row: AgentListRow,
  filters: import("@multica/core/agents/stores").AgentListFilters,
  search: string,
): boolean {
  ...
  if (
    ...
    !row.agent.model.toLowerCase().includes(q) &&
    !(row.runtime?.name.toLowerCase().includes(q) ?? false) &&
...
export function AgentsPage(_props: AgentsPageProps = {}) {
  ...
  const rows = useMemo<AgentListRow[]>(() => {
    ...
    return filtered;
  }, [scopeRows, filters, search, sortField, sortDirection]);
