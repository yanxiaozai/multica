...
CREATE UNIQUE INDEX idx_spec_document_epic_kind
    ON spec_document(epic_id, doc_kind)
    WHERE epic_id IS NOT NULL;

CREATE UNIQUE INDEX idx_spec_document_module_kind
    ON spec_document(module_id, doc_kind)
    WHERE module_id IS NOT NULL;

CREATE TABLE spec_issue_state (
    id UUID PRIMARY KEY DEFAULT gen_random_uuid(),
    workspace_id UUID NOT NULL REFERENCES workspace(id) ON DELETE CASCADE,
...
CREATE TABLE spec_issue_mapping (
    id UUID PRIMARY KEY DEFAULT gen_random_uuid(),
    workspace_id UUID NOT NULL REFERENCES workspace(id) ON DELETE CASCADE,
    issue_id UUID NOT NULL REFERENCES issue(id) ON DELETE CASCADE,
...
CREATE INDEX idx_spec_epic_workspace ON spec_epic(workspace_id, key);
CREATE INDEX idx_spec_module_workspace ON spec_module(workspace_id, epic_id, key);
CREATE INDEX idx_spec_document_workspace ON spec_document(workspace_id, module_id, doc_kind);
CREATE INDEX idx_spec_issue_state_workspace ON spec_issue_state(workspace_id, issue_id);
CREATE INDEX idx_spec_issue_mapping_workspace ON spec_issue_mapping(workspace_id, issue_id);
CREATE INDEX idx_spec_decision_workspace ON spec_decision(workspace_id, created_at DESC);
